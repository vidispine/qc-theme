...
export function useConfiguration({ type }) {
  let context = React.useContext(ConfigurationContext);
  if (type) {
    const { resources } = context;
    const resource = resources.find(({ type: t }) => t === type);
